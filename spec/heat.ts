...
export function evaluate(
  note: { due: string | null; bankedUntil?: string | null },
  now: Date = new Date()
): Burn {
  const t = minutesToDue(note.due, now);
  const banked = note.bankedUntil && new Date(note.bankedUntil).getTime() > now.getTime();
  // Banking freezes the front at the value it held when banking began.
  const burn = banked ? burnFor(minutesToDue(note.due, new Date(note.bankedUntil!))) : burnFor(t);
  return {
    burn,
    warmth: warmthFor(t),
